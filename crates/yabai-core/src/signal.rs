...
/// optional `label`/`app`/`title` filters. `app`/`title` are stored verbatim;
/// regex matching is deferred (the C compiles POSIX `REG_EXTENDED` regexes).
...
impl Signal {
    ...
    /// reproduce the C `daemon_fail` text. `app`/`title` accept the `!=` form
    /// (exclusion) but the pattern is not yet compiled or matched.
